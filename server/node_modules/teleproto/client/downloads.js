...
const FilePool_1 = require("../network/FilePool");
...
/** @hidden */
async function downloadFile(client, inputLocation, { outputFile = undefined, partSizeKb = undefined, fileSize = undefined, progressCallback = undefined, dcId = undefined, }) {
    var _a, _b;
    ...
    try {
        if (totalBytes <= 0) {
            await streamSequential(client, location, targetDc, partSize, writer, abort.signal, reportProgress);
        }
        else {
            await streamParallel(client, location, targetDc, partSize, totalBytes, writer, abort.signal, reportProgress);
        }
...
async function streamSequential(client, location, dcId, partSize, writer, signal, onBytes) {
    let idx = 0;
    while (true) {
        if (signal.aborted)
            return;
        const offset = (0, big_integer_1.default)(idx).multiply(partSize);
        const data = await client._filePool.getFile(dcId, location, offset, partSize, signal);
        if (data.length > 0) {
...
async function streamParallel(client, location, dcId, partSize, totalBytes, writer, signal, onBytes) {
    const totalParts = Math.max(1, Math.ceil(totalBytes / partSize));
    const ordered = new OrderedWriter_1.OrderedWriter(writer);
    const inflight = new OrderedWriter_1.BoundedSemaphore(Math.max(1, client._filePool.opts.inflightPerDc));
    let firstError;
    const tasks = [];
    ...
    if (firstError && !(firstError instanceof FilePool_1.FilePoolAbortError)) {
        throw firstError;
    }
}
/** @hidden */
